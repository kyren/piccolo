    ...
    math.set_field(
        ...
        callback("abs", &ctx, |_, v: Value| {
            if let Value::Integer(i) = v {
                Some(Value::Integer(i.unsigned_abs() as i64))
            } else {
                Some(v.to_number()?.abs().into())
            }
        }),
    );
...
#[cfg(feature = "rng")]
fn load_rand<'gc>(ctx: Context<'gc>, math: Table<'gc>) {
    use alloc::rc::Rc;
...
fn load_compare<'gc>(ctx: Context<'gc>, math: Table<'gc>) {
    math.set_field(
        ctx,
        "max",
...
#[cfg(feature = "std")]
fn load_float<'gc>(ctx: Context<'gc>, math: Table<'gc>) {
...
#[cfg(feature = "std")]
fn load_trig<'gc>(ctx: Context<'gc>, math: Table<'gc>) {
    math.set_field(
        ctx,
        "asin",
