...
do -- Floating point formatting
    ...
    -- Note: PRLua does not support %F (uppercase float; NAN/INF instead of nan/inf).
...
do -- alternate integer formats (hex, octal)
...
-- TODO: implement load for "%q" round-trip testing

do -- "%q" round-tripping fo strings
    local s = ""
...
[[cdefghijklmnopqrstuvwxyz{|}~\127"]])
    ...
    assert_eq(string.format("%q", "\0" .. "\1"), "\"\\0\\1\"")
    assert_eq(string.format("%q", "\0" .. "1"), "\"\\0001\"")
end
